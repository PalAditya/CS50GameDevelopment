...
function Brick:init(x, y, j)
    ...
    self.inPlay = true
	self.a=Powerup(self.x,self.y,self.poweruptype)
...
function Brick:render()
    ...
    if self.inPlay then
		if self.special==0 then
			...
			love.graphics.draw(gTextures['main'], love.graphics.newQuad(160,48,32,16,gTextures['main']:getDimensions()),self.x,self.y)
		end
    end
	if self.powerup~=0 then
		self.a:render()
	end
	if self.inPlay==false and self.powerup==1 then
		--a=Powerup(self.x,self.y)
		self.powerup=2
	end	
end
