	...
	for i=1,8,1 do
		for j=1,8,1 do
			if i-1>=1 then
				...
				if self:swap(self.board.tiles[i][j],self.board.tiles[i-1][j]) then
			...
			if j-1>=1 then
				...
				if self:swap(self.board.tiles[i][j],self.board.tiles[i][j-1]) then
					self:swap(temp1,temp2)
					return true
				end
				self:swap(temp1,temp2)
...
--Try shufling 20 times
	for i=1,20,1 do
		local x1=math.random(1,8)
		local y1=math.random(1,8)
		local x2=math.random(1,8)
		local y2=math.random(1,8)
		--[[local temp1=self.board.tiles[x1][y1]
		local temp2=self.board.tiles[x2][y2]]--
		self:swap(self.board.tiles[x1][y1],self.board.tiles[x2][y2])
	end
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
				...
				if not self:swap(self.highlightedTile,self.board.tiles[y][x]) then
					self:swap(temp1,temp2)
					--self.canInput=false
				else
...
function PlayState:swap(a,b)
    ...
    b.gridY = tempY
    -- swap tiles in the tiles table
    self.board.tiles[a.gridY][a.gridX] =a
	self.board.tiles[b.gridY][b.gridX] = b
